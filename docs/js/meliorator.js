...
(function($) {
            ...
            tooltip: {
                show: true,
                 content: function(label, xval, yval, flotItem){                        
                        return label + " | " + xval + " : " + yval; // manually constructing this prevents the bug in tooltip plugin that breaks when there are multiple series. 
                }
            }
        }
    ...
    this.makePieChart = function(data, domainField, rangeFields, container) {
        ...
        var options = {
            ...
            tooltip: {
                show: true,
                content: function(label, xval, yval, flotItem){                        
                        return label + " : " + Math.round(flotItem.datapoint[0]) + "%"; // manually constructing this prevents the bug in tooltip plugin that breaks when there are multiple series. 
                }
            }                 
        }
    ...
    this.makeAnalyticsPanel = function(data, labels, panelClass) {
        ...
        var aggregationKinds = ['AVERAGE', 'SUM', 'COUNT', 'MINIMUM', 'MAXIMUM', 'NONE'];
